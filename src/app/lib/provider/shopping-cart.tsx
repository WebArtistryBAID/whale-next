...
export interface ShoppingCart {
    items: LocalOrderedItem[]
    addItem: (itemType: HydratedItemType, appliedOptions: SerializableOptionItem[], amount: number) => LocalOrderedItem
...
export function ShoppingCartProvider({children}: { children: ReactNode }) {
    const [items, setItems] = useState<LocalOrderedItem[]>([])
    const [onSiteMode, setOnSiteMode] = useState(false)

    function addItem(itemType: HydratedItemType, appliedOptions: SerializableOptionItem[], amount: number): LocalOrderedItem {
        const item: LocalOrderedItem = {
    ...
    function getTotalItems(): number {
        return items.map((item: LocalOrderedItem) => item.amount)
